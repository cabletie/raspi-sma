...
int cc,debug = 0,verbose=0, daylight=0;
...
/*  Check if all data done and past sunset or before sunrise 
 Returns true if:
 * time is between sunrise and sunset or daylight option is true
 */
{
  char sunrise[25];
  char sunset[25];
  ...
  if( daylight == 1) 
  {
    if( debug==1 ) printf( "Returning is_light because daylight=%d\n", daylight );
    return 1;
  }
  ...
  {
    if( debug==1 ) printf( "Returning is_light=1 because no almanac data\n" );
  ...
  if( debug==1 ) printf( "Returning is_light=0\n" );
  return 0;
}
...
void PrintHelp()
{
    ...
    printf( "  -day,--daylight                          Pretend it's daylight\n" );
    printf( "\n" );
    ...
    printf( "  -np,   --nopost                          Don't post anything to PVOutput.org\n");
    printf( "\n\n" );
}

/* Init Config to default values */
int ReadCommandConfig( ConfType *conf, int argc, char **argv, char * datefrom, char * dateto, int * verbose, int * debug, int * repost, int * test, int * install, int * update , int * daylight )
{
    int	i;
	...
	else if ((strcmp(argv[i],"-day")==0)||(strcmp(argv[i],"--daylight")==0)) (*daylight) = 1;
	else if ((strcmp(argv[i],"-np")==0)||(strcmp(argv[i],"--nopost")==0)) conf->nopost = 1;
...
int main(int argc, char **argv)
{
   ...
   struct archdata_type
   {
    ...
    if( ReadCommandConfig( &conf, argc, argv, datefrom, dateto, &verbose, &debug, &repost, &test, &install, &update, &daylight ) < 0 )
        exit(0);
    // read Config file
    if( GetConfig( &conf ) < 0 )
        exit(-1);
    ...
    if( verbose == 1 ) printf("is_light() =  %u\n",isLight );
